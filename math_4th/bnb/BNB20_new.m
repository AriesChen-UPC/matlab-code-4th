function [errmsg,Z,X,t,c,fail] = BNB20_new(fun,x0,intcon,xl,xu,a,b,aeq,beq,nonlc,setts,opts,varargin);
...
% The updated version was made by Dingyu Xue, 
% Northeastern University, China, e-mail: xuedingyu@mail.neu.edu.cn
% Mainly on
% (1) function handles and structure are supported for objective functions
% (2) pre- and post-manipulations on the integer variables are made such
% that the function is better suitable to mixed integer programming problems.
% (3) all the modifications are marked by "by DYX" 

% The followings are bypassed in the updated version by DYX
...
Z=[]; X=[]; t=0; c=0; fail=0;
% the following statement was modified by DYX to allow structure problem
if ~strcmp(class(fun),'struct') & nargin<2, errmsg='BNB needs at least 2 input arguments.'; return; end;

% the following statements were added by DYX to allow structured problems 
if strcmp(class(fun),'struct'), 
    fun0=fun;
    if nargin>1, setts=x0; end
    if isfield(fun0,'objective'), fun=fun0.objective; 
    else, errmsg='Objective function not specified'; return; end
    if isfield(fun0,'x0'), x0=fun0.x0;
    else, errmsg='initial value vector not specified'; return; end
    if isfield(fun0,'intcon'), 
        intcon=fun0.intcon; xstat=zeros(size(x0)); xstat(intcon)=1;
    else, errmsg='intcon not specified'; return; end
    if isfield(fun0,'lb'), xl=fun0.lb; end
    if isfield(fun0,'ub'), xu=fun0.ub; end
    if isfield(fun0,'Aineq'), a=fun0.Aineq; end
    if isfield(fun0,'Bineq'), b=fun0.Bineq; end
    if isfield(fun0,'Aeq'), aeq=fun0.Aeq; end
    if isfield(fun0,'Beq'), beq=fun0.Beq; end
    if isfield(fun0,'nonlcon'), nonlc=fun0.nonlcon; end
    if isfield(fun0,'options'), opts=fun0.options; end 
end

% the folloing if statement was added to allow function handles by DYX
...
if isempty(x0), errmsg='No x0 found.'; return;
    ...
    % the following statement was added by DYX
    x0=x0(:);
    if ~isnumeric(x0) | ~isreal(x0) | size(x0,2)>1
    errmsg='x0 must be a real column vector.'; return;
end; end

% the following five statements were added by DYX
%ix=(intcon==1); 

%the following logic conditions were modified by DYX
xstatus=zeros(size(x0));
if (nargin>2|exist('intcon')==1) & ~isempty(intcon)
   xstatus(intcon)=1; 
%     if isnumeric(intcon) & isreal(intcon) & all(size(intcon)<=size(x0))
%         if all(xstat==round(xstat) & 0<=xstat & xstat<=2)
%             xstatus(1:size(xstat))=xstat;
%         else errmsg='xstatus must consist of the integers 0,1 and 2.'; return; end;
%     else errmsg='xstatus must be a real column vector the same size as x0.'; return; end;
end;
ix=intcon;
xlb=zeros(size(x0));
xlb(find(xstatus==0))=-inf;
if (nargin>3 | exist('xl')==1) & ~isempty(xl)
    if isnumeric(xl) & isreal(xl) & all(size(xl)<=size(x0))
        %the following statement was added by DYX
        xl(ix)=ceil(xl(ix)); xl=xl(:);
        xlb(1:size(xl,1))=xl;
...
if (nargin>4 | exist('xu')==1) & ~isempty(xu)
    if isnumeric(xu) & isreal(xu) & all(size(xu)<=size(x0))
        %the following statement was added by DYX
        xu(ix)=floor(xu(ix)); xu=xu(:);
        xub(1:size(xu,1))=xu;
...
nonlcon='';
if (nargin>9 | exist('nonlc')==1) & ~isempty(nonlc)
    if strcmp(class(nonlc),'function_handle'), nonlcon=nonlc;
    else
        if ischar(nonlc), nonlcon=nonlc;
    else errmsg='fun must be a string.'; return; end;
end, end;
settings = [0 0];
if (nargin>10 & exist('setts')) & ~isempty(setts)
...
if (nargin>11 | exist('opts')==1) & ~isempty(opts)
    if isstruct(opts)
        if isfield(opts,'MaxSQPIter')
            ...
            else, maxSQPiter=1000; opts.maxSQPiter=MaxSQPIter; return; end;
        end;
...
%the following statements are commented off to allow function handles by DYX
...
%     if evalreturn==1, return; end;
%     if size(C,2)>1 | size(Ceq,2)>1, errmsg='C and Ceq must be column vectors.'; return; end;
% end;
...
% the next statement is added by DYX for compatibility to recent versions
% of MATLAB
errx=optimget(options,'TolX'); if length(errx)==0, errx=1e-5; end
handleupdate=[];
...
options.Display='none';optionsdisplay=getfield(options,'Display');
...
% the next statement is added by DYX
X(ix)=round(X(ix));
